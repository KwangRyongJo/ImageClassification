# https://www.kaggle.com/mjbhobe/fruits-360-keras-99-test-accuracy/data
...
train_folder_root = 'D:/2021/17-01-2021/cassavadata/train_raw-467-tt/train'
test_folder_root = 'D:/2021/17-01-2021/cassavadata/train_raw-467-tt/test'
...
def display_sample(sample_images, sample_labels, sample_predictions=None, num_rows=8, num_cols=8,
    ...
    with sns.axes_style("whitegrid"):
        ...
        f, ax = plt.subplots(num_rows, num_cols, figsize=((16, 9) if fig_size is None else fig_size),
        ...
        for r in range(num_rows):
            for c in range(num_cols):
                ...
                ax[r, c].axis("off")
                # show selected image
                # Image.fromarray(sample_images[image_index])
                pil_image = sample_images[image_index]
                ax[r, c].imshow(pil_image)

                if sample_predictions is None:
                    # show the actual labels in the cell title
                    title = ax[r, c].set_title(
                        "%s" % fruit_categories[sample_labels[image_index]])
                else:
                    ...
                    if prediction_matches_true:
                        ...
                        title = '%s/%s' % (fruit_categories[true_label]
                                           [:5], fruit_categories[pred_label][:5])
                        title_color = 'r'
                    # display cell title
...
def show_plots(history, plot_title=None, fig_size=None):
    ...
    val_loss_vals = history['val_loss'] if 'val_loss' in history.keys(
    ) else None
    ...
    if acc_vals is None:
        ...
        acc_vals = history['accuracy'] if 'accuracy' in history.keys(
        ) else None
    ...
    if val_acc_vals is None:
        ...
        val_acc_vals = history['val_accuracy'] if 'val_accuracy' in history.keys(
        ) else None
    ...
    with sns.axes_style("darkgrid"):
        ...
        fig, ax = plt.subplots(nrows=1, ncols=col_count, figsize=(
...
NUM_EPOCHS, BATCH_SIZE = 150, 128  # epoch = 30
...
train_generator = train_datagen.flow_from_directory(
    train_folder_root,
    # all images will be resizes to this size
...
labels = np.argmax(labels, axis=1)  # pip3 install myo-python==0.2.2
display_sample(images[:16], labels[:16], num_rows=4, num_cols=4,
               plot_title='Sample Training Data', fig_size=(16, 9))


K.clear_session()  # tf.reset_default_graph()

best_model_finetuned_path = 'model-xcep-9.h5'

# NOTE: will download the weights for imagenet
use_l2_loss = True
IMAGE_SIZE = [299, 299]
vgg16_base = tf.keras.applications.Xception(
...
if l2_loss is not None:
    print('Using l2_loss_lambda = %f' % l2_loss_lambda)

top_model = Sequential()


top_model.add(GlobalAveragePooling2D(input_shape=vgg16_base.output_shape[1:]))
Dropout(0.30)
Dense(2048, activation='relu', kernel_regularizer=l2_loss)
Dropout(0.10)
Dense(1024, activation='relu', kernel_regularizer=l2_loss)
Dropout(0.10)
Dense(1024, activation='relu', kernel_regularizer=l2_loss)
Dropout(0.10)
Dense(1024, activation='relu', kernel_regularizer=l2_loss)
...
Dense(1024, activation='relu', kernel_regularizer=l2_loss)
Dropout(0.10)
Dense(512, activation='relu',  kernel_regularizer=l2_loss)
Dropout(0.10)
...
show_plots(hist.history, "Keras Model")

# evaluate performance on train, cross-val & test datasets
loss, acc = model.evaluate_generator(
    train_generator, steps=train_steps, verbose=1, workers=3)
print('Training data  -> loss: %.3f, acc: %.3f' % (loss, acc))
loss, acc = model.evaluate_generator(
    eval_generator, steps=eval_steps, verbose=1, workers=3)
print('Cross-val data -> loss: %.3f, acc: %.3f' % (loss, acc))
loss, acc = model.evaluate_generator(
    test_generator, steps=test_steps, verbose=1, workers=3)
print('Testing data   -> loss: %.3f, acc: %.3f' % (loss, acc))

model.load_weights(best_model_finetuned_path)

# save_keras_model(model, "cassava_disease_xception", MODEL_SAVE_DIR)
# del model

# model = load_keras_model("cassava_disease_xception", MODEL_SAVE_DIR)
# print(model.summary())
...
# print('We got %d of %d correct (or %.3f accuracy)' %
#       (correct_pred_count, len(all_labels), test_acc))

# # display sample predictions
# images, labels = test_generator.next()
# labels = np.argmax(labels, axis=1)
# predictions = np.argmax(model.predict(images), axis=1)
# display_sample(images[:16], labels[:16], sample_predictions=predictions[:16], num_rows=4, num_cols=4,
#                plot_title='Sample Predictions on Test Dataset', fig_size=(16, 9))
